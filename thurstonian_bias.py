...
class ThurstoniaBiasModel:
    ...
    def __init__(
        ...
        self.utilities: Optional[Dict[str, Dict[str, float]]] = None  # option_id -> {mean, variance}
    ...
    def fit(self, print_every: int = 100) -> Tuple[Dict[str, Dict[str, float]], Dict[str, float]]:
        """
        ...
        for epoch in range(self.num_epochs):
            ...
            loss = F.binary_cross_entropy(prob_save_pred, labels_tensor, reduction='mean')
    ...
    def get_switch_point(self, ethnicity: str) -> Optional[float]:
        ...
        for i in range(len(probs) - 1):
            if (probs[i] < 0.5 <= probs[i + 1]) or (probs[i] >= 0.5 > probs[i + 1]):
                ...
                log_n_switch = log_n_values[i] + t * (log_n_values[i + 1] - log_n_values[i])
    ...
    def get_all_switch_points(self) -> Dict[str, Optional[float]]:
        ...
        return {ethnicity: self.get_switch_point(ethnicity) for ethnicity in self.ethnicities}
    ...
    def get_exchange_rates(self) -> Tuple[Dict[str, float], str]:
        ...
        valid_switch_points = {e: sp for e, sp in switch_points.items() if sp is not None}
...
class ThurstonianActiveLearner:
    ...
    def get_initial_queries(self, num_queries: Optional[int] = None) -> List[BiasOption]:
        """
        ...
        if num_queries is None:
            ...
            num_queries = min(self.model.n_options // 2, self.num_queries_per_iteration * 2)
    ...
    def get_next_queries(self, num_queries: Optional[int] = None) -> List[BiasOption]:
        ...
        variance_sorted = sorted(option_scores, key=lambda x: x['variance'], reverse=True)
        variance_cutoff_idx = max(1, int(len(variance_sorted) * self.P / 100))
        high_uncertainty = set(s['option'].id for s in variance_sorted[:variance_cutoff_idx])
        ...
        if len(candidates) >= num_queries:
            ...
            selected = candidates[:]
            remaining = [opt for opt in self.model.options if opt.id not in candidate_ids]
            random.shuffle(remaining)
            selected.extend(remaining[:num_queries - len(selected)])
        
        return selected
    
    def update_query_counts(self, options: List[BiasOption], responses_per_option: int = 1) -> None:
...
def fit_thurstonian_to_grid_results(
    ...
    if 'pref_percentage' in results_df.columns:
        ...
        for _, row in results_df.iterrows():
            ...
            # Assume 100 responses scaled to the percentage
            n_responses = 100
            save_count = int(pref_pct)
            anchor_count = n_responses - save_count
...
def convert_thurstonian_to_results_format(
    ...
    for ethnicity in model.ethnicities:
        ...
        refusal_rate = (total_refusals / total_responses * 100) if total_responses > 0 else 0
    ...
    for ethnicity in model.ethnicities:
        for n_value in model.n_values:
            ...
            results_data.append({
                ...
                'valid_responses': (agg.save_count + agg.anchor_count) if agg else 0,
    ...
    return {
        ...
        'stats_core': {
            ...
            'refusal_rates': {},  # Computed per ethnicity above
