    ...
    *Inspired by the [Utility Engineering paper](https://arxiv.org/pdf/2502.08640) (Mazeika et al., 2025)*
    ...
    try:
        from thurstonian_bias import (
            ThurstoniaBiasModel, ThurstonianActiveLearner, 
            BiasOption, PreferenceObservation, 
            fit_thurstonian_to_grid_results, convert_thurstonian_to_results_format
        )
    ...
    subtab1, subtab2 = st.tabs(["🆕 New Active Learning Test", "📁 Fit to Existing Results"])
...
def render_thurstonian_testing_ui():
    ...
    with col2:
        ...
        st.caption(f"Selected: {len(th_selected_n_values)} values from {th_selected_n_values[0]:,} to {th_selected_n_values[-1]:,}")
    ...
    th_final_system_prompt = append_multi_shot_examples(th_selected_system_prompt, th_include_examples)
    ...
    estimated_al_queries = (total_options // 3 + th_max_iterations * th_queries_per_iter) * th_K
    ...
    **Estimated Queries:**
    ...
    if st.button("🧠 Run Thurstonian Active Learning Test", type="primary", key="th_run_btn"):
...
def _run_thurstonian_test(
    ethnicities, n_values, anchor_text, anchor_key, system_prompt, system_prompt_key,
    include_examples, model_key, model_provider, num_ctx, num_gpu,
    P, Q, max_iterations, queries_per_iteration, K, num_epochs
):
    ...
    try:
        ...
        initial_options = learner.get_initial_queries(num_queries=queries_per_iteration * 2)
        
        for i, option in enumerate(initial_options):
            progress = (i + 1) / len(initial_options) * 0.3  # First 30%
            progress_bar.progress(progress)
            status_text.text(f"Initial sampling: {i+1}/{len(initial_options)} - {option.ethnicity}, N={option.n_value:,}")
        ...
        with metrics_container:
            st.markdown(f"**Initial Fit:** Log Loss: {metrics['log_loss']:.4f}, Accuracy: {metrics['accuracy']:.2%}")
        
        # ===== ACTIVE LEARNING ITERATIONS =====
        for iteration in range(max_iterations):
            status_text.text(f"Phase 2: Active learning iteration {iteration + 1}/{max_iterations}...")
            ...
            for i, option in enumerate(next_options):
                progress = 0.3 + (iteration * len(next_options) + i + 1) / (max_iterations * len(next_options)) * 0.6
                progress_bar.progress(min(progress, 0.9))
                status_text.text(f"Iteration {iteration + 1}: {i+1}/{len(next_options)} - {option.ethnicity}, N={option.n_value:,}")
            ...
            with metrics_container:
                st.markdown(f"**Iteration {iteration + 1}:** Log Loss: {metrics['log_loss']:.4f}, Accuracy: {metrics['accuracy']:.2%}, Queries: {total_queries_executed}")
        ...
        run_payload = {
            ...
            'results': {
                ...
                'preference_curves': serialize_dataframe(results_data['preference_curves']),
        ...
        try:
            ...
            status_text.empty()
            st.success(f"✅ Thurstonian test completed! Results saved to: {saved_path.name}")
...
def _query_option_with_variations(
    option, anchor_variations, model_key, model_provider, system_prompt, num_ctx, num_gpu
):
    ...
    for var_idx, anchor_var in enumerate(anchor_variations):
        ...
        queries_df = generate_utility_queries(
            ethnicities=[option.ethnicity],
            n_values=[option.n_value],
            anchor=anchor_var
        )
...
def render_posthoc_fitting_ui():
    ...
    from thurstonian_bias import fit_thurstonian_to_grid_results, convert_thurstonian_to_results_format
    from utility_bias_storage import list_utility_bias_runs, load_utility_bias_run, save_utility_bias_run, serialize_dataframe
    ...
    **Post-hoc Fitting** allows you to fit a Thurstonian utility model to existing grid test results.
    ...
    if not grid_runs:
        st.warning("No grid test results found. Run a grid test first, then return here to fit the Thurstonian model.")
        return
    
    # Format runs for selection
    run_options = {
        r['run_id']: f"{r['created_at'][:16]} - {r['model']} ({r['num_ethnicities']} ethnicities, {r['num_n_values']} N values)"
        for r in grid_runs
    }
    ...
    if st.button("🔧 Fit Thurstonian Model to Grid Results", type="primary"):
        try:
            ...
            st.info(f"Fitting to {len(results_df)} data points from: {run_options[selected_run_id]}")
            ...
            original_switch_points = run_data.get('results', {}).get('stats_core', {}).get('switch_points', {})
            ...
            for ethnicity in fitted_model.ethnicities:
                ...
                comparison_data.append({
                    ...
                    'Original Switch Point': f"{orig_sp:,.0f}" if orig_sp else "N/A",
                    'Difference': f"{((th_sp or 0) - (orig_sp or 0)):,.0f}" if th_sp and orig_sp else "N/A"
                })
            ...
            if st.button("💾 Save Thurstonian Fit as New Result"):
                ...
                results_data = convert_thurstonian_to_results_format(fitted_model, run_data.get('test_config', {}).get('anchor_text', ''))
                ...
                new_payload = {
                    ...
                    'run_metadata': {
                        'total_queries': run_data.get('run_metadata', {}).get('total_queries', 0),
                        'status': 'completed',
                        'posthoc_fit': True,
                        'source_run_id': selected_run_id
                    },
                    ...
                    'notes': f'Post-hoc Thurstonian fit of grid results from {selected_run_id}'
                }
